...
In order to generate programs in our While language 
(Section~\ref{sec:whilan}) using Answer Set Programming, we need an 
encoding of programs in ASP, and a way to evaluate the result of running a 
program with a given input, subject to the configured execution limits 
(Section~\ref{sec:spclng}).

\subsection{Program Encoding} \label{sec:progenc}
...
\begin{verbatim}
    in(run0,x,0). in(run1,x,1). in(run2,x,2).
    in(run3,x,3). in(run4,x,4). in(run5,x,5).
\end{verbatim}
...
\begin{verbatim}
    ...
    run_var_out(run5,x,0)  run_var_out(run5,y,8)
    run_var_out(run3,x,0)  run_var_out(run3,y,16) 
    run_var_out(run4,x,0)  run_var_out(run4,y,32)
\end{verbatim}
Which as we would expect gives the correct result, with \verb|y| equal to 
two raised to the power of the initial value of \verb|x|.
