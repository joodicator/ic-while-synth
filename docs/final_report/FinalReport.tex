...
See Chapter~\ref{sec:whilan} for a formal description of this language.
...
programs (see Chapter~\ref{sec:bkgsyn}). Most can be classified as 
...
Programming (see Chapter~\ref{sec:bkgasp}), which was previously used for 
...
\begin{itemize}
    ...
    in speed of generation. See Chapter~\ref{sec:gensub}.
    ...
    increase in speed of generation. See Chapter~\ref{sec:gensub}.
\end{itemize}
\clearpage
...
\section{Programs in the While Language} \label{sec:whilan}

\subsection{Language Definiton}
...
\subsection{Program Encoding in ASP} \label{sec:progenc}
...
Below is an example of the program from Section~\ref{sec:intro}, which 
...
Note that although these parameters are related to the ones given in 
Section~\ref{sec:spclng}, Task Specification Language, they are not given 
in the same form here, for reasons of efficiency. The Specification 
Language will be used by higher-level components of the system.
...
from the example at the end of the previous 
section \ref{sec:progenc}, and with the following additional facts:
\begin{verbatim}
    in(run0,x,0).
    in(run1,x,1).
    in(run2,x,2).
...
\section{Synthesis of Programs from User Examples} \label{sec:synexm}
...
\section{Automatic Generation of Examples} \label{sec:genexm}

\subsection{Extensions to the Task Specification Language}
The specification language from Chapter~\ref{sec:synexm} is extended with 
...
\section{Separate Synthesis of Subprograms} \label{sec:gensub}

\subsection{Extensions to the Task Specification Language}

\subsection{Separate Synthesis of Loop Bodies}
